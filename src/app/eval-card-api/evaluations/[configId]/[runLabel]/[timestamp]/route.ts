...
function parseModelId(rawModelId: string) {
...
function transformEvalResult(evalResult: WevalResult) {
    const flattenedResults = [];

    for (const prompt of evalResult.config.prompts) {
        for (const modelId of evalResult.effectiveModels) {
            if (modelId === 'ideal') continue; // Skip the ideal response

            const responseText = evalResult.allFinalAssistantResponses?.[prompt.id]?.[modelId];
            if (!responseText) continue; // Skip if no response exists

            const coverageResult = evalResult.evaluationResults?.llmCoverageScores?.[prompt.id]?.[modelId];
            const avgCoverageScore = coverageResult?.avgCoverageExtent ?? 0;

            const modelInfo = parseModelId(modelId);

            const result = {
                schema_version: '0.0.1-dtef-transformed',
                evaluation_id: `${evalResult.configId}/${evalResult.runLabel}/${evalResult.timestamp}/${prompt.id}/${modelId}`,
                model: {
                    model_info: {
                        name: modelInfo.name,
                    ...
                    inference_settings: {
                        ...
                        generation_args: {
                            temperature: prompt.temperature ?? evalResult.config.temperature ?? null,
                        },
                    },
                    eval_model_id: modelId, // Add our precise ID for traceability
                },
                prompt_config: {
                ...
                instance: {
                    ...
                    sample_identifier: { // We don't have this, so we construct a placeholder
                        dataset_name: evalResult.configId,
                        hf_repo: 'collect-intel/dtef-configs',
                        hf_split: 'custom',
                        hf_index: -1,
                    },
                },
                ...
                eval_tags: (evalResult.config.tags || []).map(tag => ({
                    name: tag,
...
export async function GET(
    ...
    try {
        ...
        const transformedData = transformEvalResult(resultData);
