...
async function runPairGeneration(body: any) {
  ...
  setContext('generatePairs', { configId });
  ...
  try {
    ...
    // Generate the pairs (no Netlify blob context needed - falls back to env var auth)
    ...
    if (result.anchorModelMissing) {
      ...
      logger.error(`Failed to generate pairs for ${configId}: anchor model missing.`);
      return;
    }
    ...
    captureError(error, { configId, message: error.message });
...
export async function POST(req: NextRequest) {
  ...
  // Fire and forget
  runPairGeneration(body).catch(err => {
    console.error('[generate-pairs-background] Error:', err);
    captureError(err);
  });
